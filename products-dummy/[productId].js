...
export async function getStaticProps(context) {
  // rút ngắn 2 dòng bên dưới:
  // const segmentProductId = context.params.productId
  ...
  const product = data.products.find(
    (product) => product.id === segmentProductId
  );
...
export async function getStaticPaths() {
  ...
  return {
    ...
    */
    ...
    // Fallback Pages
    //fallback: true, // render pages even though it was not added to paths, just in time
    fallback: false, // render 404 page if it was not added to paths
    //fallback: "blocking", // loading the page takes a bit longer but we get the entire finished page. do not need if(!abc)
  };
}
